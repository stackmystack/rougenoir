...
use crate::{
    CachedTree, Node, NodeAdapter, TreeCallbacks,
...
pub struct IntoIter<K, V, C, A: Allocator = Global>(CachedTree<K, V, C, A>);
...
impl<K, V, C: TreeCallbacks<Key = K, Value = V>, A: Allocator> IntoIterator
...
impl<'a, K, V, C, A: Allocator> IntoIterator for &'a mut CachedTree<K, V, C, A> {
...
impl<'a, K, V> Iterator for IterMut<'a, K, V> {
    ...
    fn next(&mut self) -> Option<(&'a K, &'a mut V)> {
        self.inner.next().map(|mut n| {
            ...
            // 'a, per `CachedTree::iter_mut`'s contract.
...
impl<'a, K: Ord + Copy, V: Copy, C: TreeCallbacks<Key = K, Value = V>, A: Allocator>
    Extend<(&'a K, &'a V)> for CachedTree<K, V, C, A>
{
...
impl<K: Ord, V, C: TreeCallbacks<Key = K, Value = V> + Default, A: Allocator + Default>
    FromIterator<(K, V)> for CachedTree<K, V, C, A>
{
    /// Constructs a `CachedTree<K, V>` from an iterator of key-value pairs.
    ///
    /// If the iterator produces any pairs with equal keys,
    /// all but one of the corresponding values will be dropped.
    fn from_iter<T: IntoIterator<Item = (K, V)>>(iter: T) -> CachedTree<K, V, C, A> {
        let inputs: Vec<_> = iter.into_iter().collect();
        let mut res = CachedTree::with_callbacks_in(Default::default(), A::default());
        for (k, v) in inputs {
            res.insert(k, v);
        }
        res
